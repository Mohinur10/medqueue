...
export default function AdminLayout({ children }: { children: React.ReactNode }) { const { currentUser, activeRole } = useMedQueue();
 const router = useRouter();
 const [isAuthorized, setIsAuthorized] = useState(false);
 useEffect(() => { 
/*  If not authenticated or role is not super_admin, push to unauthorized page */
if (!currentUser || activeRole !== 'super_admin') { router.replace('/unauthorized');
