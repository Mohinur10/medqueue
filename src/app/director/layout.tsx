...
export default function DirectorLayout({ children }: { children: React.ReactNode }) { const { currentUser, activeRole } = useMedQueue();
 const router = useRouter();
 const [isAuthorized, setIsAuthorized] = useState(false);
 useEffect(() => { 
/* Check if user session has clinic_director scope */
 if (!currentUser || activeRole !== 'clinic_director') { router.replace('/unauthorized');
